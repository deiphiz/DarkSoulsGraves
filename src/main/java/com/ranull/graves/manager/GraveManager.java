...
public class GraveManager {
    ...
    public void removeGraveMax(ConcurrentMap<Location, GraveInventory> playerGraves) {
    	long highest = 0;
    	GraveInventory oldestGrave = null;
    	
        for (ConcurrentMap.Entry<Location, GraveInventory> entry : playerGraves.entrySet()) {
            GraveInventory graveInventory = entry.getValue();
            
            long graveTime = System.currentTimeMillis() - graveInventory.getCreatedTime();
            Bukkit.getLogger().info(Long.toString(graveTime));
            
            if (graveTime >= highest){
            	oldestGrave = graveInventory;
            	highest = graveTime;
            }
        }
        dropGrave(oldestGrave);
        dropExperience(oldestGrave);
        removeHologram(oldestGrave);
        replaceGrave(oldestGrave);
        removeGrave(oldestGrave);
    }
